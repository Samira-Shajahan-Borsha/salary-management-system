...
//abstract, so other classes can use same abstract matheds- calsal, salperhr
...
class  Manager  extends  Employee {
 ...
 static double salperhr=0;  //static, so can be used and update by all methods in this class
 double hours;
...
//CLASS SENIORSALESPERSON
 ...
 //implements interface Bonus to use bonus value
...
class SeniorAccountant extends Accountant implements Bonus{
 ...
 //this bonus value can be inherited by many classes
 interface Bonus{
...
class Staffs{
 ...
 public  void  displaySalaries() throws ZeroStaffsException { //add stuff zeo exception
		if(noofstaffs==0){
			throw new ZeroStaffsException("Exception");
		}else{
			for ( int i=0; i<noofstaffs; i++){
		try{
			System.out.println(stf[i].getName()+" earned "+stf[i].calculateSalary()+" Dollars " );
		}catch (ZeroResultException e){
			System.out.println( "salary per hour is zero, update salary per hour from menu");
 }
 }
 }
 }
...
public  class  Salaries3{
 
 public  static  void  main ( String [] args ) {

//file operator for welcome text
 try {
 File welobj = new File("Welcome.txt");
  ...
  while (myReader.hasNextLine()) {
      ...
      System.out.println("Welcome Header File Missing");
    }
	
//object for staff class, to call addemployee and display methods
 Staffs p =  new  Staffs ();
 Scanner sc = new Scanner(System.in);
 
 int exit1=0; // prepared to exit for  MENU switch case
 
	do{ //do loop to repeat the menu switch case
		
		// file operator for MENU list text
	try {
		...
		while (myReader.hasNextLine()) {
			...
			System.out.println("Menu Header File Missing");
    }
 char ch = sc.next().charAt(0);
 switch(ch){
	 case 'a' :
	 int exit2=0; //Prepare to Exit ffrom add Employee switch case
		 do{  //do loop to repeat the add employee switch case
	      try {
				...
				System.out.println("Add Employee Header File Missing");
			}
	      ...
	      switch(ch1){
		    ...
		    case 'e' :
			  ...
			  System.out.println("Caught exception - hours cannot be negative");
				 ...
				 System.out.println("Caught exception - Senior Employee Age must be greater than 35");
			 }
				break;
			case 'f' :
			  ...
			  System.out.println("Caught exception - hours cannot be negative");
				...
				break;
				case 'g' :
				//case to exit the add employee switch case, and back to MENU list switch case
				exit2=1;  //to make do while loop condition invalid
				break;
			    }
		  }while(exit2==0);
     break;
		
     case 'b' : // to enter update salary 
	 
		int exit3 = 0; // prepare Exit for the update salary switch we
		do{// repeat update salary switch case until exit
	  try {
			...
			while (myReader.hasNextLine()) {
				...
				System.out.println("Update Salary Header File Missing");
		}
		char ch2 = sc.next().charAt(0);
	    switch(ch2){ //update salary switch case
			case 'a' :
			...
			case 'd' :
				...
				break;
			case 'e' :
				exit3=1; //make exit update salary and back to MENU
				break;
				}		 
	      }while (exit3==0);
	  break;	 
	case 'c' : // Display salary from MENU
	try{
    	p.displaySalaries();
        ...
        break;
	case 'd' : //Exit from MENU and close app
	    exit1=1;
        break;
       }
	 }while(exit1==0);
